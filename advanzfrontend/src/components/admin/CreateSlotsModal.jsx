...
const CreateSlotsModal = ({ isOpen, onClose, onCreateSlots }) => {
    ...
    useEffect(() => {
        const fetchDoctors = async () => {
            try {
                const response = await axios.get(`${API_URL}/myadmin/api/get/slots/all/doctor/`);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
          ...
          const response = await axios.post(`${API_URL}/doctor/api/create/slots/`, payload);
      
          console.log(response.data);
      
        } catch (error) {
          setError("Error creating slots. Please try again.");
          console.error("Error creating slots:", error);
        }
      };
    return (
        <div className={`modal ${isOpen ? "modal-open fixed inset-0 flex justify-center items-center bg-black bg-opacity-75" : "hidden"}`}>
            ...
            <div className="modal-container bg-white rounded-lg shadow-lg w-[30rem]">
                <div className="modal-header bg-gradient-to-r from-blue-900 to-blue-300 p-4 rounded-t-lg flex justify-between items-center">
                    ...
                    <button className="modal-close text-gray-300 hover:text-gray-400" onClick={onClose}>
                        &times;
                    </button>
                </div>
